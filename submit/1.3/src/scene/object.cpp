...
{   
    ...
    T<<1,0,0,center[0],
        ...
        0,0,1,center[2],
        0,0,0,1;
        const float x_rad    = radians(x_angle);
        const float y_rad    = radians(y_angle);
        const float z_rad    = radians(z_angle);
    Eigen::Matrix4f Rx,Ry,Rz;
    Rx<<1,0,0,0,
        0,cos(x_rad),-sin(x_rad),0,
        0,sin(x_rad),cos(x_rad),0,
        0,0,0,1;
    Ry<<cos(y_rad),0,sin(y_rad),0,
        0,1,0,0,
        -sin(y_rad),0,cos(y_rad),0,
        0,0,0,1;
    Rz<<cos(z_rad),-sin(z_rad),0,0,
